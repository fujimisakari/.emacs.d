...
(global-unset-key "\M-t")           ; transpose-wordsはタイポしがちなので外す
...
(bind-key "M-'" 'my/region-to-single-quote region-bindings-mode-map)   ; ''で囲む
(bind-key "M-\"" 'my/region-to-double-quote region-bindings-mode-map)  ; ""で囲む
(bind-key "M-9" 'my/region-to-bracket region-bindings-mode-map)        ; ()で囲む
(bind-key "M-[" 'my/region-to-square-bracket region-bindings-mode-map) ; []で囲む
(bind-key "M-l" 'my/region-to-clear region-bindings-mode-map)          ; 囲みをクリア
...
(bind-key "C-k" 'kill-line)                   ; カーソル位置より右を削除
(bind-key* "C-t" 'my/other-window-or-split)   ; ウィンドウを切り替える
(bind-key "C-h" 'delete-backward-char)        ; C-hをバックスペースに割り当て
(bind-key "C-m" 'newline-and-indent)          ; 改行してインデント
...
(bind-key "C-M-;" 'ace-window)                ; ウィンドウ選択
...
(bind-key* "M-p" 'my/scroll-up-in-place)      ; カーソル維持スクロール(上)
(bind-key* "M-n" 'my/scroll-down-in-place)    ; カーソル維持スクロール(下)
(bind-key "M-o" 'swiper-thing-at-point)       ; swiperの起動(thing-at-point)
(bind-key "M-x" 'counsel-M-x)                 ; counselでM-x
(bind-key "M-y" 'counsel-yank-pop)            ; kill-ringの内容を取り出す
(bind-key "M-Y" 'my/insert-image-like-logsec) ; org-modeへインライン画像貼り付け
(bind-key "M-/" 'hippie-expand)               ; 略語展開・補完
...
(bind-key "M-SPC" 'bm-toggle)                 ; 現在行に色をつけて記録
...
(bind-key "C-l w" 'my/normalize-spaces-in-region) ; 連続スペースを1スペースへ
(bind-key "C-l W" 'whitespace-cleanup)        ; TABを空白に置換
(bind-key "C-l k" 'my/keitai-hankaku-katakana-region) ; 全角カナを半角カナに
...
(bind-key "C-l a" 'my/counsel-ag-with-ignore) ; counsel-ag検索(ignore付き)
(bind-key "C-l A" 'counsel-ag)                ; counsel-ag検索
(bind-key "C-l d" 'my/dired-open-current-directory) ; バッファをdiredで開く
...
(bind-key "C-l s" 'my/switch-to-scratch/current-buffer) ; *scratch*に移動
(bind-key "C-l S" 'my/swap-window-positions)  ; ウィンドウを入れ替える
(bind-key "C-l t" 'my/www-page-title)         ; pageタイトル取得
(bind-key "C-l z" 'my/elscreen-set-custom-screen) ; screenを定位置に(custom)
(bind-key "C-l Z" 'my/elscreen-set-default-screen) ; screenを定位置に(default)
...
(bind-key "C-l C-'" 'ispell-word)             ; スペルから候補を表示
(bind-key "C-l C-." 'my/insert-arrow)         ; → を追加
(bind-key "C-l C-;" 'my/google-translate-enja-or-jaen) ; google翻訳
(bind-key* "C-l C-M-l" 'highlight-symbol-remove-all)   ; highlight表示を解除
...
(bind-key "C-l g b" 'magit-branch-checkout)   ; git branch
...
(bind-key "C-c C-d" 'lispxmp emacs-lisp-mode-map)              ; 実行結果を注釈

;; python-mode
(with-eval-after-load 'python
  (bind-key "C-c f" 'py-autopep8-region python-mode-map)   ; コード整形
  (bind-key "C-c i" 'py-isort-buffer python-mode-map)      ; import整形
  (bind-key "C-c d" 'my/python-docstring-comment python-mode-map)) ; docstring生成
...
(with-eval-after-load 'markdown-mode
  (bind-key "C-c C-s" 'my/markdown-header-list markdown-mode-map)
  (bind-key "C-c C-c" 'my/markdown-preview-by-eww markdown-mode-map)
...
(with-eval-after-load 'org
  ...
  (bind-key "C-," nil org-mode-map)
  (bind-key "C-c C-t" 'my/convert-text-to-org-table org-mode-map)
  (bind-key "M-S-<left>" 'org-promote-subtree org-mode-map)
  (bind-key "M-S-<right>" 'org-demote-subtree org-mode-map)
  (bind-key "M-S-<up>" 'org-move-subtree-up org-mode-map)
  (bind-key "M-S-<down>" 'org-move-subtree-down org-mode-map))

;; dired-mode
(bind-key "C-f" 'my/dired-open-in-accordance-with-situation dired-mode-map)
(bind-key "C-M-f" 'my/dired-open-directory-in-new-buffer dired-mode-map)
(bind-key "C-M-b" 'dired-up-directory dired-mode-map)
(bind-key "C-M-r" 'my/dired-remove-by-shell dired-mode-map)
(bind-key "C-t" 'my/other-window-or-split dired-mode-map)
(bind-key "/" 'dired-ex-isearch dired-mode-map)
